...
const Sidebar = () => {
    // 1. Extraemos 'user' además de 'logout'
    const { user, logout } = useAuth(); 

    // 2. Creamos las validaciones de roles
    const isJuca = user?.tipo_usuario === 'Juca';
    const isMenuUser = ['Juca', 'Administrativo', 'Profesor'].includes(user?.tipo_usuario);
        ...
        <div className="d-flex flex-column flex-shrink-0 p-3 bg-dark text-light h-100" style={{ width: '250px' }}>
            ...
            <div className="flex-grow-1 overflow-y-auto hide-scrollbar">
                <Nav variant="pills" className="flex-column mb-auto">
                    ...
                    {/* --- PARTE MENÚ: SÓLO JUCA, ADMINISTRATIVO O PROFESOR --- */}
                    {isMenuUser && (
                        <>
                            <div className="mb-2 mt-2">
                                <span className="text-light text-uppercase small fw-bold px-3">Menú</span>
                            </div>
                            <Nav.Item>
                                <Link to="/home" className={`nav-link link-primary text-white`}>
                                    <i className="bi bi-house-door me-2"></i> Home
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
                                <Link to="/mis_citas" className={`nav-link link-warning text-white`}>
                                    <i className="bi bi-calendar-event me-2"></i> Mis Citas
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
                                <Link to="/codigo" className={`nav-link link-secondary text-white`}>
                                    <i className="bi bi-qr-code-scan me-2"></i> Código QR
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
                                <Link to="/autos" className={`nav-link link-secondary text-white`}>
                                    <i className="bi bi-car-front me-2"></i> Cajones disponibles
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
                                <Link to="/estado_cajones" className={`nav-link link-secondary text-white`}>
                                    <i className="bi bi-geo-alt me-2"></i> Estado de Cajones
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
                                <Link to="/terminos" className={`nav-link link-secondary text-white`}>
                                    <i className="bi bi-file-text me-2"></i> Términos
                                </Link>
                            </Nav.Item>
                        </>
                    )}

                    {/* --- PARTE ADMINISTRACIÓN: SÓLO ROL JUCA --- */}
                    {isJuca && (
                        <>
                            <hr />
                            ...
                            <Nav.Item>
                                <Link to="/crud_citas" className={`nav-link link-primary text-white`}>
                                    <i className="bi bi-journal-text me-2"></i> Gestión de Citas
                                </Link>
                            </Nav.Item>
                            <Nav.Item>
